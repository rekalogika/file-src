<?php

declare(strict_types=1);

/*
 * This file is part of rekalogika/file-src package.
 *
 * (c) Priyadi Iman Nurcahyo <https://rekalogika.dev>
 *
 * For the full copyright and license information, please view the LICENSE file
 * that was distributed with this source code.
 */

namespace Rekalogika\Domain\File\Association\Entity\Tree;

use Rekalogika\Collections\Decorator\Trait\IteratorAggregateDecoratorTrait;
use Rekalogika\Collections\Decorator\Trait\ReadableCollectionDecoratorTrait;

/**
 * Directory in a tree
 */
trait DirectoryTrait
{
    /**
     * @use ReadableCollectionDecoratorTrait<int,AbstractNode>
     */
    use ReadableCollectionDecoratorTrait;

    /**
     * @use IteratorAggregateDecoratorTrait<int,AbstractNode>
     */
    use IteratorAggregateDecoratorTrait;
}
